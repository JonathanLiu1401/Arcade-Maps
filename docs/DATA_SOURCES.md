...
These land in `data/enrichment.json` via `scrapers/enrich.py`, not in `arcades.json`. Images are the exception: the weekly crawl keeps `skip_pictures=1`, so no picture URLs come from `data_raw/ziv.json` itself. The 2,003 ZIv venue photos in the shipped file were collected by `scrapers/photos.py` and `scrapers/chain_photos.py`, which query without that flag, and join through the ZIv page URL rather than through the bulk rows.
...
**What the shipped file actually contains.** The key list above is the full set the builder can emit, not the set on disk. The current `data/enrichment.json` holds **9,795** entries against **13,540** arcades: `hours_text` (5,259), `info_text` (4,241), `website` (4,095), `machine_prices` (3,664) and `images` / `image_tier` (5,196), plus the per-entry `sources` and `enriched_at` metadata. Every text field is tagged `ziv`. There is still no `transport`, `fav_count`, `game_prices`, `game_versions`, `pay_type`, `price_text` or `hours`: `bemanicn_rows_contributed` is 3,193 of 3,812, and every one of those contributions is a photo (see sections 6 and 11).
...
**Coverage today: 5,196 of 13,540 arcades (38.4%)**, by contributing source: BemaniCN 3,193, ZIv 2,003, Wikimedia Commons **0**. By country it is very uneven: China 51.1%, United Kingdom 37.4%, United States 35.2%, Japan **3.3%**, Taiwan 3.4%.
