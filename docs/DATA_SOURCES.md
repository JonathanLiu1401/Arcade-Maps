...
- **Politeness:** every scraper sleeps after each request (0.4 s by default, `DEFAULT_SLEEP` in `scrapers/common.py`). Ordinary failures retry 3 times (1s/2s/4s). HTTP 429/500/502/503/504 retry 8 times with a longer cap (5s ... 120s) because eagate's Imperva edge has 503'd GitHub runners (2026-08-17 weekly). Requests are sequential, never parallel fan-out against a single host. The shared User-Agent is currently a fixed desktop-browser string (`common.USER_AGENT`); switching to a project-identifying UA is desirable but untested against the eagate WAF. The eagate scraper warms the per-game index page and keeps a cookie jar so Imperva can issue `visid`/`nlbi` cookies before `list.html`.
...
**Infrastructure note:** eagate is fronted by an Imperva/Incapsula WAF-CDN. It is reachable from US IPs and GitHub-hosted runners as of July 2026, but WAF behavior changes without notice. The 2026-08-17 weekly Action got HTTP 503 on the first `list.html` request from `ubuntu-latest` (three tries, 1s/2s/4s, then abort). The scraper now warms `index.html?gkey=` with a cookie jar, sends a Referer, and `common.fetch` waits out 429/5xx longer.
...
These land in `data/enrichment.json` via `scrapers/enrich.py`, not in `arcades.json`. Images are the exception: the weekly crawl keeps `skip_pictures=1`, so no picture URLs come from `data_raw/ziv.json` itself. The 2,001 ZIv venue photos in the shipped file were collected by `scrapers/photos.py` and `scrapers/chain_photos.py`, which query without that flag, and join through the ZIv page URL rather than through the bulk rows.
...
| Effect | 4,200 rows placed at address level; China distinct coordinates 2,090 -> 5,305; worst pile-up on one point 69 venues -> 6 |
...
**What the shipped file actually contains.** The key list above is the full set the builder can emit, not the set on disk. The current `data/enrichment.json` holds **9,571** entries against **11,967** arcades: `hours_text` (5,865), `info_text` (4,231), `website` (4,122), `machine_prices` (3,656) and `images` / `image_tier` (5,101), plus the per-entry `sources` and `enriched_at` metadata. Every text field is tagged `ziv`. There is still no `transport`, `fav_count`, `game_prices`, `game_versions`, `pay_type`, `price_text` or `hours`: `bemanicn_rows_contributed` is 3,100 of 3,812, and every one of those contributions is a photo (see sections 6 and 11).
...
**Coverage today: 5,101 of 11,967 arcades (42.6%)**, by contributing source: BemaniCN 3,100, ZIv 2,001, Wikimedia Commons **0**. By country it is very uneven: China 65.3%, United Kingdom 37.6%, United States 35.3%, Japan **3.3%**, Taiwan 3.4%.

Commons is harvested but does not ship. `data_raw/chain_photos.json` holds 48 Commons image records with full credit and licence, and none of them reach `data/enrichment.json`: the join in `photos.index_by_source_url()` only emits `ziv:<id>` and `bemanicn:<id>` keys parsed out of `page_url`, so a `commons.wikimedia.org` URL yields no key and `photos_for_arcade()` never returns it. Re-joining them needs a stable key that is not the arcade id (ids are reassigned on every merge), so the count above says 0 rather than 22.
...
| Wikimedia Commons geosearch | REJECT. Raw hits are majority tourist/transport geography; no hit was confirmed to be the arcade. (The category-walked, hand-reviewed Commons path in `chain_photos.py` is a different thing: it harvests 48 image records into `data_raw/chain_photos.json`, but none of them currently reach `data/enrichment.json` - see the join note in the coverage section) |
...
All sources are re-scraped by a single GitHub Action: weekly cron, Monday 18:00 UTC, plus `workflow_dispatch` for manual runs. The job runs with `permissions: contents: write`, commits changed files under `data/`, `data_raw/`, and `mymaps/` with a bot identity, and skips the commit when nothing changed. Empty parses and ZIv empty-country 200s fail the job (parser/spelling bugs). A source that is merely down after retries keeps its previous raw file and lets the other sources commit. FX is the same shape: total feed failure keeps the previous rates file and does not fail the job. Both scrape targets (location.am-all.net, p.eagate.573.jp) are empirically reachable from GitHub-hosted US runners as of July 2026, but eagate's Imperva edge has returned 503 to those runners (2026-08-17).
