 ...
 "arcades": [
  {"id": int (sequential after sorting by country,name,addr;
   ...
   "approx_level": "address"|"street"|"district"|"city" (OPTIONAL -
          ...
          derived. Current build: address 5,758, district 7, city 2.
...
- Not every arcade has anything enrichable (9,795 of 13,540 in the current build); a sparse side file avoids null-padding 13k rows.
...
The official WAHLAP endpoint returns name, address and province but NO
coordinates and NO machine count (the committed `data_raw/china_wahlap_*.json`
rows carry name / address / games / notes and nothing else - counts on a
Chinese pin come from BemaniCN, never from WAHLAP), and BemaniCN's public
endpoints are addresses + game lists only (its coordinate layers are
login-walled).
...
BD-09 and GCJ-02 to WGS-84, and commits the answer. 5,758 rows are now
