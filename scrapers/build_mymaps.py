...
CSVs (name,address,latitude,longitude,games,cabs,game_counts,source)
mirror each layer and include coordinate-less members (blank lat/lng);
game_counts is a "slug:n;slug:n" string, empty when no source reported
machine counts for the store. KMZ features
...
def placemark(a, style_id, flag_games):
    ...
    if a.get("game_counts"):
        desc.append("counts: " + ", ".join(
            "%s:%d" % (g, a["game_counts"][g])
            for g in sorted(a["game_counts"])))
    if a["cabs"]:
...
def write_csv(path, arcades):
    with open(path, "w", encoding="utf-8-sig", newline="") as f:
        ...
        w.writerow(["name", "address", "latitude", "longitude",
                    "games", "cabs", "game_counts", "source"])
        for a in arcades:
            gc = a.get("game_counts") or {}
            w.writerow([
                a["name"], a["addr"],
                "" if a["lat"] is None else "%.7f" % a["lat"],
                "" if a["lng"] is None else "%.7f" % a["lng"],
                ";".join(a["games"]), ";".join(a["cabs"]),
                ";".join("%s:%d" % (g, gc[g]) for g in sorted(gc)),
                ";".join(a["src"]),
            ])


def prune_stale_kmz(out_dir, written):
    """Delete KMZ files this run did NOT write but a previous run did.

    A layer's file name depends on its size: <=MAX_FEATURES features is a
    single "<slug>.kmz", more is "<slug>_a.kmz", "<slug>_b.kmz", ... When a
    layer crosses that threshold (China did: 622 -> 6,357 features once the
    approximate placements landed) the OLD single-file KMZ is never
    overwritten and stays on disk holding the previous run's smaller, stale
    pin set. mymaps/README.md then lists only the parts while the directory
    also offers the stale file, and the weekly Actions job (`git add
    mymaps/`) commits it - so anyone importing the un-suffixed name gets old
    data forever.

    Only KMZ names this module itself generates are considered, so CSVs,
    README.md and china_all_addresses.csv can never be touched. Returns the
    deleted names.
    """
    candidates = set()
    for layer in LAYERS:
        slug = layer["slug"]
        candidates.add(slug + ".kmz")
        for i in range(26):
            candidates.add("%s_%s.kmz" % (slug, chr(ord("a") + i)))
    removed = []
    for fname in sorted(candidates - set(written)):
        path = os.path.join(out_dir, fname)
        if os.path.exists(path):
            os.remove(path)
            removed.append(fname)
            print("removed stale %s (not written by this run)" % fname,
                  file=sys.stderr)
    return removed
...
def run(data_dir, out_dir):
    ...
    for layer in LAYERS:
        ...
        written_kmz.update(f for f, _ in files)
    ...
    prune_stale_kmz(out_dir, written_kmz)
