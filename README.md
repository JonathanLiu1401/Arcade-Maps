...
**Current dataset (last rebuild 2026-08-24):** 12,529 arcades across 69 countries (191 cross-source duplicates were unified by romanization-aware matching, so a Japanese official listing and its romaji community twin now share one pin; a further 124 where two sources geocoded the same venue more than 120 m apart - ALL.Net's fullwidth "アミューズメントパークＭＧ西条" and e-amusement's "アミューズメントパークMG西条店" were one store drawn as two icons; and 22 in Hong Kong and Macau, where the official English listing and BemaniCN's Chinese one share no characters at all and are bridged by the street number, the operator's Latin branding, or the Cantonese reading of the Chinese name - 碧富遊戲機 on 和宜合道 is PIK FU GAME CENTRE on Wo Yi Hop Road). Sources include SEGA ALL.Net, Konami eagate, WAHLAP China, BemaniCN, Round1 USA, and Zenius-I-Vanisher (65 country queries, plus a United States per-series crawl).

## Live map

**https://jonathanliu1401.github.io/Arcade-Maps/**

World view on the default CARTO Voyager basemap (OpenStreetMap tiles are the automatic fallback if CARTO fails, and a Settings > Display toggle). 12,297 plottable pins of 12,529 stores (the rest are address-only and live in the no-coords list). A metric/imperial scale bar sits above the attribution. Other / unknown-count markers are violet with a halo, not grey, so they stay visible on Voyager beige. Legend shapes tint to the first selected game:

![Arcade Maps world view with game filters](docs/screenshot.png)

Akihabara at street zoom. Each store is a chibi tier icon tinted by its games, not a numbered circle:

![Akihabara with tiered arcade markers](docs/readme-akihabara.jpg)

Store panel for GiGO Akihabara 3: venue photos, cab counts, hours, and measured prices.

![Store detail panel for GiGO Akihabara 3](docs/readme-place.jpg)

The same store on a phone (bottom sheet) and with the UI in Japanese:

![Mobile bottom sheet for GiGO Akihabara 3](docs/readme-mobile-place.jpg)

![Japanese UI for the same store](docs/readme-ja.jpg)

Tokyo with the on-map legend expanded. Marker shapes follow the first selected game's colour; the scale bar is in the corner:

![Expanded marker legend and scale bar over Tokyo](docs/readme-legend.jpg)
...
- Settings you keep: turn individual data sources off, flatten the marker size ramp so every icon draws at 25px (the tier shapes stay), pick CARTO Voyager or OpenStreetMap tiles (Voyager is the default; OSM is also the automatic fallback), and disable location features entirely. Choices persist per device in localStorage and never travel in a shared link. The on-map legend is a click-to-expand chip (shapes tint to the first selected game), not a setting. A metric/imperial scale bar sits on the map
...
- Venue photos in the detail panel: 4,995 of 12,529 arcades (39.9%) now carry a real photo of that venue, shown as a slideshow when there is more than one and credited to the community member who took it. When a store has none, the panel falls back to a CC-licensed photo of one of its cabinets from Wikimedia Commons (with the required author/license credit rendered on the image), then to a game-colour banner. Coverage is uneven by design of the sources, not by choice: about two thirds in China (mirrored BemaniCN thumbnails), about a third in the United Kingdom and the United States, and only 3.3% in Japan. See "Photo coverage" below for why Japan is the outlier
...
- No backend, no API keys: static HTML + vendored Leaflet 1.9.4 + Leaflet.markercluster 1.5.3 on GitHub Pages, CARTO Voyager raster tiles by default with OpenStreetMap as automatic fallback if CARTO fails (and as a Settings > Display choice). The one optional exception is the Google Places photo path, which stays a complete no-op unless somebody adds their own key ([docs/GOOGLE_PHOTOS.md](docs/GOOGLE_PHOTOS.md))
...
- **Photo coverage is thin in Japan and Taiwan** (3.3% and about 8%) because the chains that dominate those markets forbid embedding their store imagery. See "Photo coverage" above.
...
|- docs/
|  |- screenshot.png     README world-view capture (CARTO Voyager)
|  |- readme-akihabara.jpg  Akihabara street zoom, chibi markers
|  |- readme-place.jpg   GiGO Akihabara 3 store panel
|  |- readme-mobile-place.jpg  phone bottom sheet
|  |- readme-ja.jpg      Japanese UI for the same store
|  |- readme-legend.jpg  expanded legend + scale bar over Tokyo
...
- [OpenStreetMap](https://www.openstreetmap.org/copyright) contributors and [CARTO](https://carto.com/attributions) - base map tiles (CARTO Voyager default, with OpenStreetMap tiles as automatic fallback)
