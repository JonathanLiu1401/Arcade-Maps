...
**Current dataset (last rebuild):** 13,510 arcades across 68 countries (191 cross-source duplicates were unified by romanization-aware matching, so a Japanese official listing and its romaji community twin now share one pin; a further 150 where two sources geocoded the same venue hundreds of metres apart - ALL.Net's fullwidth "アミューズメントパークＭＧ西条" and e-amusement's "アミューズメントパークMG西条店" were one store drawn as two icons; and 14 in Hong Kong, where the official English listing and BemaniCN's Chinese one share no characters and are bridged by the street number, or by the operator's own Latin branding plus the locality when the two sources cite different faces of the same corner building). Sources include SEGA ALL.Net, Konami eagate, WAHLAP China, BemaniCN, Round1 USA, and Zenius-I-Vanisher (65 country queries, plus a United States per-series crawl that also tracks Pump It Up / ITG / StepManiaX and related cabs under `other`).
